<?php
/**
 * Sniffs_OSR0_WhiteSpace_OperatorSpacingSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class OSR0_Sniffs_WhiteSpace_OperatorSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_BITWISE_AND)
        {
            ...
            else
            {
                ...
                {
                    $error = '在操作符 "&" 前必须有且只能有 1 个空格; 而不是 %s';
                    $data = array($found);
            ...
            else
            {
                ...
                {
                    $error = '在操作符 "&" 后必须有且只能有 1 个空格; 而不是 %s';
                    $data = array($found);
        ...
        if ($tokens[($stackPtr - 1)]['code'] !== T_WHITESPACE)
        {
            $error = "在操作符 \"$operator\" 前必须至少有 1 个空格";
        ...
        {
            if (isset(PHP_CodeSniffer_Tokens::$assignmentTokens[$tokens[$stackPtr]['code']]) === false)
            {
                ...
                {
                    $error = '在操作符 "%s" 前必须有且只能有 1 个空格; 而不是 %s';
                    $data = array(
                        $operator,
                        $found,
                    );
        ...
        if ($tokens[($stackPtr + 1)]['code'] !== T_WHITESPACE)
        {
            $error = "在操作符 \"$operator\" 后必须有且只能有 1 个空格";
        ...
        else
        {
            ...
            {
                $error = '在操作符 "%s" 后必须有且只能有 1 个空格; 而不是 %s';
                $data = array(
                    $operator,
                    $found,
                );
